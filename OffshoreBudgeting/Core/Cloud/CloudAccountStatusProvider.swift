...
final class CloudAccountStatusProvider: ObservableObject {
    ...
    func resolveAvailability(forceRefresh: Bool = false) async -> Bool {
        let ok = await CloudStatus.shared.resolveAvailability(
            forceRefresh: forceRefresh,
            client: CloudClient(containerIdentifier: Self.containerIdentifier),
            coreDataZoneID: coreDataZoneID
        )
    ...
    func invalidateCache() {
        CloudStatus.shared.invalidateCache()
